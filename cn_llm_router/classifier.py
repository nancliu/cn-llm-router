...
RULE_KEYWORDS: dict[str, list[str]] = {
    "程序编码": ["写代码", "写个函数", "写函数", "实现", "python", "go语言", "shell", "脚本", "算法", "代码片段", "补全代码", "生成代码"],
    "前端UX设计": ["前端", "html", "css", "布局", "页面", "ui", "ux", "样式", "组件", "hover", "深色主题", "登录页"],
    "文书撰写": ["周报", "月报", "纪要", "公文", "通知", "方案", "报告", "合同", "文书", "策划", "总结", "邮件正文"],
    "数据分析": ["数据分析", "统计", "透视", "sql", "图表", "可视化", "趋势", "指标", "销售数据", "订单量", "分析一下"],
    ...
    "创意生成": ["起名", "起名字", "名字", "想一个", "创意", "广告语", "文案", "头脑风暴", "故事开头", "点子", "取名"],
    "代码审查与调试": ["报错", "bug", "审查", "调试", "性能优化", "优化一下", "为什么错", "error", "异常", "单元测试"],
}
...
class Classifier:
    ...
    def _build_prompt(self, text: str) -> str:
        ...
        return (
            "你是任务分类器。把用户的请求归入下列 12 个类别之一，并判断复杂度（低/中/高：低=简单直接，"
            "中=常规任务，高=多步骤/专业深度）。\n\n类别定义：\n"
            + "\n".join(lines)
