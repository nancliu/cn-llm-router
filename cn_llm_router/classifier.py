...
class Classifier:
    ...
    def _build_prompt(self, text: str) -> str:
        ...
        return (
            ...
            + "\n".join(lines)
            + "\n\n判定规则（类别与复杂度独立判断）：\n"
            "- 按任务的【目的与产出物】定类别，不因含编程/实现词汇（虚拟滚动、状态机、组件、框架、"
            "API、函数、脚本、渲染、实现）而误判为程序编码：目标是页面/组件交互与表现 → 前端UX设计；"
            "目标是流程/编排方案 → Agent编排；目标是综述式回答 → 知识问答/检索\n"
            "- “梳理XX路线/脉络/流派/机制”（未指定具体文档）是综述型问答 → 知识问答/检索；"
            "仅当要求阅读/总结/提取【指定】文档内容 → 长文档处理\n"
            "- 复杂度锚点仅用于定复杂度，不影响类别判断。\n\n"
            "复杂度（低/中/高）判断锚点：\n"
